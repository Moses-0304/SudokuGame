...
public class SudokuController {
        ...
        for (int row = 0; row < SudokuUtilities.GRID_SIZE; row++) {
            for (int col = 0; col < SudokuUtilities.GRID_SIZE; col++) {
                ...
                if(newBoard[row][col][0] != 0) {
                    model.getCell(row, col).setShownFirst(true);
                }
    ...
    /**
     * Initiates the process to save the current state of the game to a .sudoku file.
     * <p>
     * The method displays a file chooser dialog to allow the user to specify the file
     * location. If a location is chosen, it serializes the current game model to that location.
     * If any error occurs during the saving process, it displays an error message to the user.
     * </p>
     */
    public void saveGame() {
        ...
        if (file != null) {
            try {
                fileHandler.serializeToFile(model, file.getAbsolutePath());
            } catch (Exception ex) {
                ex.printStackTrace();
    ...
    /**
     * Initiates the process to load a previously saved game state from a .sudoku file.
     * <p>
     * The method displays a file chooser dialog to allow the user to specify the file
     * from which to load the game. If a valid game file is chosen, it deserializes the
     * game model from that file and updates the view accordingly. If any error occurs
     * during the loading process, it displays an error message to the user.
     * </p>
     */
    public void loadGame() {
        ...
        if (file != null) {
            try {
                model = fileHandler.deSerializeFromFile(file.getAbsolutePath());
                for (int row = 0; row < SudokuUtilities.GRID_SIZE; row++) {
                    for (int col = 0; col < SudokuUtilities.GRID_SIZE; col++) {
